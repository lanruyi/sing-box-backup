...
func stopDriver(t *testing.T) {
	...
	require.NoError(t, err)
	service, err := windows.OpenService(manager, serviceNameW, windows.SERVICE_STOP)
...
// Image unlock can lag the SCM stop by a moment, so retry the write.
func plantTamperedDriver(t *testing.T, target string, planted []byte) {
	t.Helper()
	require.NoError(t, os.MkdirAll(filepath.Dir(target), 0o755))
	require.Eventually(t, func() bool {
		return os.WriteFile(target, planted, 0o644) == nil
	}, 10*time.Second, 100*time.Millisecond, "driver image still locked after service stop")
}
